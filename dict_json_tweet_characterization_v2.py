    ...
    for line in tweets_file:
        try:
            ...
            if 'text' in tweet:
                ...
                if dict_userid_retweet.has_key(user_retweeted):
        ...
        try:
            ...
            if 'text' in tweet:
                ...
                for usersx_id in users_id_mention:
                        # increment the count of the selected ID
                        if dict_userid_mention.has_key(str(usersx_id)):
                           dict_userid_mention[str(user_id_comp)] +=1    

                # Retweet status
                user_retweeted =  tweet['retweeted_status']['user']['id']
                if dict_userid_retweet.has_key(user_retweeted):
                    dict_userid_retweet[str(user_id_comp)] +=1    
        except:
            # read in a line is not in JSON format (sometimes error occured)
            continue
    tweets_file.close()
...
if ( __name__ == "__main__" ):
    ...
    if(city_name in top_cities):
        ...
        # Open csv file
        users_file_csv = "tweets_characterization/users_id_name.csv"
        # csv reader from userid/username file
        reader = csv.reader(open(users_file_csv, 'r'))
